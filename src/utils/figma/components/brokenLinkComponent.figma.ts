import { BaseFileData, DEFAULT_SETTINGS, FIGMA_COMPONENT_PREFIX } from "../../constants/constants";
...
export async function createBrokenLinkComponent(parent: FrameNode) {
  ...
  await figma
    .loadFontAsync({ family: 'Inter', style: 'Bold' })
    .then(() => {
      //Component
...
export async function generateBrokenLinkInstance(caption: string): Promise<InstanceNode> {
  ...
  await figma.getNodeByIdAsync(componentData.brokenLink.id).then((node) => {
    component = node;
  });
